...
pub struct TransformResult {
    pub code: String,
    pub promoted_functions: HashSet<String>,
}
...
impl ContextAnalyzer {
    ...
    fn enter_context<F>(&mut self, context: ExprContext, f: F)
    where
        F: FnOnce(&mut Self),
    {
    ...
    fn record_context(&mut self, expr: &Expr, context: ExprContext) {
        let expr_id = ExprId::from(expr.span());
        self.expression_contexts.insert(expr_id, context);
    }

    fn record_call(&mut self, expr: &CallExpr) {
        let expr_id = ExprId::from(expr.span);
    ...
    fn detect_curry_inner(&mut self, node: &CallExpr) {
        let Callee::Expr(callee_expr) = &node.callee else {
            return;
        };

        let inner_call = match callee_expr.as_ref() {
            Expr::Call(c) => Some(c),
        ...
        if let Some(inner) = inner_call {
            self.skip_set.insert(ExprId::from(inner.span));
        }
    }
}
...
impl VisitMut for ContextAnalyzer {
    ...
    fn visit_mut_cond_expr(&mut self, node: &mut CondExpr) {
        self.enter_context(ExprContext::TernaryCondition, |analyzer| {
            analyzer.record_context(&node.test, ExprContext::TernaryCondition);
            node.test.visit_mut_with(analyzer);
        });

        self.enter_context(ExprContext::TernaryConsequent, |analyzer| {
            analyzer.record_context(&node.cons, ExprContext::TernaryConsequent);
            node.cons.visit_mut_with(analyzer);
        });

        self.enter_context(ExprContext::TernaryAlternate, |analyzer| {
            analyzer.record_context(&node.alt, ExprContext::TernaryAlternate);
    ...
    fn visit_mut_bin_expr(&mut self, node: &mut BinExpr) {
        ...
        self.enter_context(left_context, |analyzer| {
            analyzer.record_context(&node.left, left_context);
            node.left.visit_mut_with(analyzer);
        });
        self.enter_context(right_context, |analyzer| {
            analyzer.record_context(&node.right, right_context);
            node.right.visit_mut_with(analyzer);
        });
    }

    fn visit_mut_unary_expr(&mut self, node: &mut UnaryExpr) {
        self.enter_context(ExprContext::UnaryOperand, |analyzer| {
            analyzer.record_context(&node.arg, ExprContext::UnaryOperand);
            node.arg.visit_mut_with(analyzer);
        });
    }

    fn visit_mut_update_expr(&mut self, node: &mut UpdateExpr) {
        self.enter_context(ExprContext::UpdateOperand, |analyzer| {
            analyzer.record_context(&node.arg, ExprContext::UpdateOperand);
            node.arg.visit_mut_with(analyzer);
        });
    }
    ...
    fn visit_mut_assign_expr(&mut self, node: &mut AssignExpr) {
        ...
        self.enter_context(ExprContext::AssignRight, |analyzer| {
            analyzer.record_context(&node.right, ExprContext::AssignRight);
            node.right.visit_mut_with(analyzer);
        });
    }
    ...
    fn visit_mut_seq_expr(&mut self, node: &mut SeqExpr) {
        for (i, expr) in node.exprs.iter_mut().enumerate() {
            self.enter_context(ExprContext::SequenceItem(i), |analyzer| {
                analyzer.record_context(expr, ExprContext::SequenceItem(i));
    ...
    fn visit_mut_yield_expr(&mut self, node: &mut YieldExpr) {
        if let Some(arg) = &mut node.arg {
            self.enter_context(ExprContext::YieldValue, |analyzer| {
                analyzer.record_context(arg, ExprContext::YieldValue);
    ...
    fn visit_mut_await_expr(&mut self, node: &mut AwaitExpr) {
        self.enter_context(ExprContext::AwaitValue, |analyzer| {
            analyzer.record_context(&node.arg, ExprContext::AwaitValue);
            node.arg.visit_mut_with(analyzer);
        });
    }

    fn visit_mut_tagged_tpl(&mut self, node: &mut TaggedTpl) {
        self.enter_context(ExprContext::TaggedTemplate, |analyzer| {
            analyzer.record_context(&node.tag, ExprContext::TaggedTemplate);
            node.tag.visit_mut_with(analyzer);
        });
    }

    fn visit_mut_paren_expr(&mut self, node: &mut ParenExpr) {
        self.enter_context(ExprContext::Parenthesized, |analyzer| {
            analyzer.record_context(&node.expr, ExprContext::Parenthesized);
    ...
    fn visit_mut_array_lit(&mut self, node: &mut ArrayLit) {
        for (i, elem) in node.elems.iter_mut().enumerate() {
            if let Some(expr_or_spread) = elem {
                self.enter_context(ExprContext::ArrayElement(i), |analyzer| {
                    analyzer.record_context(&expr_or_spread.expr, ExprContext::ArrayElement(i));
        ...
        for prop in &mut node.props {
            match prop {
                PropOrSpread::Spread(spread) => {
                    self.enter_context(ExprContext::ObjectSpread, |analyzer| {
                        analyzer.record_context(&spread.expr, ExprContext::ObjectSpread);
                        spread.expr.visit_mut_with(analyzer);
                    });
                }
                PropOrSpread::Prop(prop) => match prop.as_mut() {
                    Prop::KeyValue(kv) => {
                        self.enter_context(ExprContext::ObjectValue, |analyzer| {
                            analyzer.record_context(&kv.value, ExprContext::ObjectValue);
    ...
    fn visit_mut_if_stmt(&mut self, node: &mut IfStmt) {
        self.enter_context(ExprContext::IfCondition, |analyzer| {
            analyzer.record_context(&node.test, ExprContext::IfCondition);
            node.test.visit_mut_with(analyzer);
        });
    ...
    fn visit_mut_while_stmt(&mut self, node: &mut WhileStmt) {
        self.enter_context(ExprContext::WhileCondition, |analyzer| {
            analyzer.record_context(&node.test, ExprContext::WhileCondition);
            node.test.visit_mut_with(analyzer);
        });
    ...
    fn visit_mut_for_stmt(&mut self, node: &mut ForStmt) {
        ...
        if let Some(test) = &mut node.test {
            self.enter_context(ExprContext::ForCondition, |analyzer| {
                analyzer.record_context(test, ExprContext::ForCondition);
                test.visit_mut_with(analyzer);
            });
        }

        if let Some(update) = &mut node.update {
            self.enter_context(ExprContext::ForUpdate, |analyzer| {
                analyzer.record_context(update, ExprContext::ForUpdate);
...
impl CheckpointInjector {
    pub fn new(runtime_import_path: String) -> Self {
        Self {
            expression_contexts: HashMap::new(),
            function_calls: HashSet::new(),
            skip_set: HashSet::new(),
    ...
    fn extract_call_info(&self, node: &CallExpr) -> (Option<String>, Option<Expr>) {
        match &node.callee {
            Callee::Expr(expr) => match expr.as_ref() {
                ...
                Expr::Call(_) => (None, None),
                Expr::Paren(_) => (None, None),
                _ => (None, None),
            },
            _ => (None, None),
        }
...
impl VisitMut for CheckpointInjector {
    ...
    fn visit_mut_await_expr(&mut self, node: &mut AwaitExpr) {
        let was_in_await = self.in_await_context;
        self.in_await_context = true;
        node.visit_mut_children_with(self);
        self.in_await_context = was_in_await;
    }
    ...
    fn visit_mut_call_expr(&mut self, node: &mut CallExpr) {
        ...
        if self.skip_set.contains(&expr_id) {
            return;
        }
    ...
    fn visit_mut_expr(&mut self, expr: &mut Expr) {
        ...
        if self.in_async_context && !self.in_constructor && !self.in_await_context {
    ...
    fn visit_mut_program(&mut self, program: &mut Program) {
        ...
        let import_decl = ImportDecl {
            ...
            src: Box::new(Str {
                span: DUMMY_SP,
                value: self.runtime_import_path.clone().into(),
                raw: None,
            }),
            type_only: false,
            with: None,
...
pub fn transform_code(
    ...
    let mut injector = CheckpointInjector::new(runtime_import_path.to_string());
    injector.set_analysis_results(
        analyzer.expression_contexts,
    ...
    let promoted_functions: HashSet<String> = injector
        .promoted_functions
        .iter()
