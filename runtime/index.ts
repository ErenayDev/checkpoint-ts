...
async function mainLoop(): Promise<void> {
  vlog(`[INDEX] Entering main loop`);
  vlog(`[INDEX] Sending ready message`);
  ...
  while (true) {
    ...
    vlog(`[INDEX] ← Received command: ${JSON.stringify(message, null, 2)}`);

    switch (message.type) {
      case "load_app": {
        vlog(`[INDEX] Load app command received`);
        ...
        vlog(`[INDEX] → Sent: version, ${Bun.version}`);
        ...
        try {
          const absolutePath =
            appPath.startsWith("/") || appPath.startsWith("file://")
              ? appPath
              : `file://${appPath}`;
          vlog(`[INDEX] Importing: ${absolutePath}`);
          await import(absolutePath);
          vlog(`[INDEX] App loaded successfully`);
          shm.sendStatusJson({
