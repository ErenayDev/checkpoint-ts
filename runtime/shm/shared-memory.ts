...
export class SharedMemory {
  ...
  static open(shmName: string): SharedMemory {
    ...
    const address = libc.symbols.mmap(null, TOTAL_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  ...
  private getSlotOffset(channel: Channel, slotIndex: number): number {
    return this.getChannelOffset(channel) + RING_BUFFER_HEADER_SIZE + slotIndex * SLOT_SIZE;
  }

  private writeToSlot(channel: Channel, slotIndex: number, data: Uint8Array): void {
  ...
  private enqueueSPSC(channel: Channel, data: Uint8Array): void {
    ...
    const writeIndexArray = new Uint32Array(this.buffer.buffer, channelOffset, 1);
    const readIndexArray = new Uint32Array(this.buffer.buffer, channelOffset + 4, 1);
    ...
    if (nextWriteIndex === currentReadIndex) {
      console.error(`[SPSC-TS] Queue full! write=${currentWriteIndex}, read=${currentReadIndex}`);
  ...
  private async waitDequeue(channel: Channel, timeoutMs: number): Promise<Uint8Array | null> {
  ...
  async waitReceiveCheckpointResponse(timeoutMs: number): Promise<Uint8Array | null> {
    return this.waitDequeue(Channel.RustToTsCheckpointResponse, timeoutMs);
  }

  async waitReceiveCheckpointResponseJson<T>(timeoutMs: number): Promise<T | null> {
  ...
  debugQueueState(): void {
    vlog("[DEBUG] ===== TypeScript Queue State =====");
    ...
    for (let channel = 0; channel < 4; channel++) {
      ...
      const count = writeIdx >= readIdx ? writeIdx - readIdx : QUEUE_CAPACITY - readIdx + writeIdx;
